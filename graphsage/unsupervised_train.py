...
def train(train_data):
    ...
    minibatch = EdgeMinibatchIterator(graph,
                                      id_map,
    ...
    if FLAGS.save_embeddings:
        ...
        if FLAGS.model == "n2v":
            ...
            train_ids = tf.constant(
                [[id_map[n]] for n in graph.nodes_iter() if not graph.node[n]['val'] and not graph.node[n]['test']],
                dtype=tf.int32)
            test_ids = tf.constant([[id_map[n]] for n in graph.nodes_iter() if graph.node[n]['val'] or graph.node[n]['test']],
                                   dtype=tf.int32)
            ...
            nodes = [n for n in graph.nodes_iter() if graph.node[n]["val"] or graph.node[n]["test"]]
            start_time = time.time()
            pairs = run_random_walks(graph, nodes, num_walks=50)
            walk_time = time.time() - start_time

            test_minibatch = EdgeMinibatchIterator(graph,
                                                   id_map,
                                                   placeholders, batch_size=FLAGS.batch_size,
                                                   max_degree=FLAGS.max_degree,
                                                   num_neg_samples=FLAGS.neg_sample_size,
