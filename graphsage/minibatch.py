...
class EdgeMinibatchIterator(object):
    ...
    def __init__(self, graph, id2idx,
        ...
        self.nodes = np.random.permutation(graph.nodes())
        ...
        if not n2v_retrain:
            ...
            self.val_edges = [e for e in graph.edges() if graph[e[0]][e[1]]['train_removed']]
        else:
            if fixed_n2v:
        ...
        print(len([n for n in graph.nodes() if not graph.node[n]['test'] and not graph.node[n]['val']]), 'train nodes')
        print(len([n for n in graph.nodes() if graph.node[n]['test'] or graph.node[n]['val']]), 'test nodes')
        self.val_set_size = len(self.val_edges)

    def _n2v_prune(self, edges):
        def is_val(n):
            return self.G.node[n]["val"] or self.G.node[n]["test"]
...
class NodeMinibatchIterator(object):
    ...
    def __init__(self, graph, id2idx,
        ...
        self.train_nodes = set(graph.nodes()).difference(self.no_train_nodes_set)
